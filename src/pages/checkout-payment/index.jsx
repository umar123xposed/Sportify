...
const CheckoutPayment = () => {
  ...
  const calculateOrderSummary = () => {
    ...
    const shippingFee = 6.00; // Example - get from config or API
  ...
  const toggleOrderSummaryModal = () => setOrderSummaryModalOpen(!orderSummaryModalOpen);
    ...
    <div className="page-wrapper pb-4">
      ...
      <Container>
        <Elements stripe={stripePromise}>
          ...
          <PaymentModal location={draft} />

        </Elements>
      </Container>

      {/* New Order Summary Modal */}
      <Modal backdrop={"static"} centered size="md" isOpen={orderSummaryModalOpen} toggle={toggleOrderSummaryModal}>
        {/* Modal Content based on the image */}
        <div className="order-summary-modal-content"> {/* Add a class for styling */}
          <div className="modal-header-custom d-flex justify-content-between align-items-center"> {/* Add a class for header styling */}
            <h2>Order Summary</h2>
            <button type="button" className="close-button" onClick={toggleOrderSummaryModal}> {/* Add a class for close button */}
              &times; {/* Close icon */}
            </button>
          </div>
          <div className="modal-body-custom"> {/* Add a class for body styling */}
              ...
              <>
                ...
                <div className="summary-details-section">
                  ...
                  <div className="summary-line-modal d-flex justify-content-between">
                    ...
                    <span className="payment-status-paid">{orderData.paymentStatus}</span> {/* Add class for "Paid" status style */}
                  </div>
                </div>
                  ...
                  <div className="package-info-modal d-flex align-items-center">
                    <div className="package-images-modal d-flex me-3">
                      {orderData.packageDetails?.items?.map((item, index) => (
                        <img key={index} src={item.imageUrl} alt={`Item ${index + 1}`} className="package-item-image-modal me-1" />
                      ))}
                    </div>
                    <div className="package-text-modal">
                      <div className="package-name-modal">{orderData.packageDetails?.name}</div>
                      <div className="package-qty-price-modal">
                        Qty: {orderData.packageDetails?.qty} | Price: ${orderData.packageDetails?.price?.toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="modal-footer-custom d-flex justify-content-end"> {/* Add a class for footer styling */}
            <button className="download-receipt-button">Download Receipt</button> {/* Add a class for button styling */}
          </div>
        </div>
      </Modal>
    </div>
