...
import { handleSportName } from '../../../redux/profileSlice';

const BasicProfileDetailModal = ({
  isOpen,
  toggle,
  ...
  const handleUpgrade = () => {
    dispatch(handleSportName({ sport: basicSportData?.sport }))
    navigate(`/parent/select-type?type=advanced&id=${searchParams.get("id")}&sport_id=${basicSportData?.id}&name=upgrade`)
  }
        ...
        <ModalBody className="modal-body-content" style={{ backgroundColor: "#5353538f" }}> {/* Use class for styling */}
            ...
            (basicSportData?.qr_code != null) && (
              <div
                ...
                onClick={() => {
                  ...
                  //   },
                  // });
                  setUser({ username: playerData?.user?.full_name, sport_name: basicSportData?.sport?.name })
          ...
          <div className="detailed-info-section mt-4">
            <div className="detailed-info-header d-flex justify-content-between align-items-center mb-2">
              ...
              {/* <button className="edit-button" onClick={onEditDetailedInfoClick}>
          ...
          <div className="mt-2 mb-3" style={{ backgroundColor: "white", borderRadius: "8px", overflow: "hidden" }}>
            ...
            <div className="info-item d-flex justify-content-between">
              ...
              <span className="item-value">{basicSportData?.profile_detail?.dominant_hand || 'N/A'}</span> {/* Adjust field name if needed */}
            </div>
            <div className="info-item d-flex justify-content-between">
              <span className="item-label">Position</span>
              {/* Join positions if it's an array */}
              <span className="item-value">{basicSportData?.profile_detail?.positions?.join(', ') || 'N/A'}</span>
            </div>
            <div className="info-item d-flex justify-content-between">
              <span className="item-label">Jersey Number</span>
              <span className="item-value">{basicSportData?.profile_detail?.jersey_no || 'N/A'}</span>
            </div>
            <div className="info-item d-flex justify-content-between no-border"> {/* Add no-border class to the last item */}
              ...
              <span className="item-value">{basicSportData?.profile_detail?.team_name || 'N/A'}</span>
            </div>
          </div>
...
export default BasicProfileDetailModal;
